...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.filtered_indices = []  # 存储过滤后的视频索引
    ...
    def create_toolbar(self) -> QGroupBox:
        ...
        self.search_input.setPlaceholderText("输入关键词过滤视频...")
        self.search_input.textChanged.connect(self.filter_video_list)
    ...
    def on_video_changed(self, index: int):
        """视频选择改变"""
        if index >= 0 and len(self.filtered_indices) > index:
    ...
    def filter_video_list(self):
        ...
        if not keyword:
            ...
            for i in range(self.data_manager.get_video_count()):
                ...
                if keyword in display_name.lower() or keyword in str(i + 1):
                    self.video_selector.addItem(f"{i + 1}. {display_name}")
                    self.filtered_indices.append(i)

        # 如果当前视频在过滤结果中，选中它
        if self.current_video_index in self.filtered_indices:
            index = self.filtered_indices.index(self.current_video_index)
            self.video_selector.setCurrentIndex(index)
